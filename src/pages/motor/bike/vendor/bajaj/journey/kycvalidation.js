...
export default async function validateKycStep(
  ...
  //  console.log("hello",pangender,pancardno,pancardDob)
  if (!pangender || !pangender || !pancardDob)
  ...
  res = await CallApi(constant.API.MOTOR.BIKE.BAJAJ.PANVERIFY, "POST", payload);

  const decrypted = res?.data?.decrypted;

  const isKycSuccess =
    decrypted?.ckycStatus === "FOUND" ||
