...
export default function StepperForm({ usersData, kycData }) {
  ...
  const GoToPayment = () => {
    sessionStorage.removeItem("godigitStepFourData");
...
// console.log("Ram",kycSuccess)
  if (kycSuccess !== null) {
    // console.log("1",kycSuccess)
    const savedData = sessionStorage.getItem("godigitStepFourData");

    if (savedData) {
      setStepThreeData(JSON.parse(savedData));
    }
