...
export default function ThankYou() {
  ...
  useEffect(() => {
    ...
    const transactionNumber = qv(router.query.transactionNumber);
    const param1 = qv(router.query.Param1);
    const param2 = qv(router.query.Param2);
    const policyurl = qv(router.query.policyurl);
    const baseKey = `thankyou_mcar_${transactionNumber}_${param1}_${param2}`;
    ...
    const run = async () => {
      ...
      try {
        const firstResponse = await CallApi(
          "/api/motor-car-godigit/car-thankyou",
          "POST",
          payload,
        );
        ...
        if (firstResponse?.status) {
          const formattedData = {
            policy: firstResponse.policyNumber,
            policyURL: firstResponse.pdflink,
          };
          ...
          try {
            ...
            sessionStorage.setItem(dataKey, JSON.stringify(formattedData));
          } catch (_) {}
        } else {
          showError(firstResponse?.message || "Something went wrong");
        }
      } catch (error) {
