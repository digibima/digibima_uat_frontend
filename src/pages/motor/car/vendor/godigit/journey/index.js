...
export default function StepperForm({ usersData, kycData }) {
  ...
  const validateFormStepOne = async () => {
    ...
    //   return false;
    // }
    step1Form.unregister("proposaldob");
  ...
  const validateFormStepThree = async (step3Form, steptwodata) => {
    ...
    try {
      ...
      const errorDesc = res?.data?.error;
      ...
      if (status === true || res === 1) {
        ...
        sessionStorage.setItem(
          "godigitCarStepFourData",
          JSON.stringify(res.data)
        );
      }
        showSuccess("Step 3 saved successfully.");
        setLoading(false); 
        // console.log("step three save",res?.data?.kyc)
  ...
  //onGoToOvdKyc start 
  const GoToOvdKyc = async () => {
    setLoading(true);

    try {
      const res = await CallApi(
        constant.API.MOTOR.CAR.GODIGIT.OVDKYC,
        "POST"
      );
      ...
      if (redirectUrl) {
        ...
        showError("KYC redirect URL not found");
      }

    } catch (error) {
      console.error("OVD KYC error:", error);
      showError("Something went wrong while starting KYC.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const GoToPayment = () => {
    ...
    router.push(
      `/motor/car/vendor/godigit/payment`
    );
  };
  ...
  if (kycSuccess !== null) {
    const savedData = sessionStorage.getItem("godigitCarStepFourData");

    if (savedData) {
      const parsed = JSON.parse(savedData);

      setStepThreeData(parsed);
      setKycError(parsed?.kyc || {});
    }
