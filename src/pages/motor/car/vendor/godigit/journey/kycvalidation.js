  ...
  try {
    ...
    // Others Verification
      ...
      if (!isAllowedImage(insurePhoto)) {
        ...
        // Check identity & address value must be same
      if (identityValue !== addressValue) {
        showError("Identity number and Address number must be same.");
        return false;
      }
      ...
      try {
        const res = await UploadDocument(
          constant.API.MOTOR.CAR.GODIGIT.UPLOADDOCUMENT,
          "POST",
          formData
        );
