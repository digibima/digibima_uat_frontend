...
export default function StepThreeForm({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (nominee.guardianname ) {
        setIsMinor(true);
        setTimeout(() => {
          step3Form.setValue("guardianname", nominee.guardianname);
          // step3Form.setValue("guardianrelation", nominee.guardianrelation);
        }, 0);
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (isMinor) {
      step3Form.register("guardianname", {
        required: "Guardian Name is required",
      });
      // step3Form.register("guardianrelation", {
      //   required: "Please select the guardian relation",
      // });
    } else {
      step3Form.unregister("guardianname");
      // step3Form.unregister("guardianrelation");
    }
  }, [isMinor, step3Form]);
