...
export default function StepTwoForm({
        ...
        <div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
              <label className="labelcls">Proof Provided for Ncb</label>
            <select
          {...register("proofprovidedforncb")}
          className={`text-sm md:col-span-2 ${inputClass}`}
          value={selectedProof}
          onChange={(e) => setSelectedProof(e.target.value)}
        >
          <option value="">Select</option>
          <option value="renewalnotice">Renewal Notice</option>
        </select>
            </div>
            <div>
              <label className="labelcls">Proof Of Document Date</label>
             <Controller
             control={control}
              name="proofOfdocumentdate"
              rules={{ required: "Date is required" }}
              render={({ field, fieldState }) => (
                <UniversalDatePicker
                  id="proofOfdocumentdate"
                  name="proofOfdocumentdate"
                  value={
                    field.value
                      ? parse(field.value, "dd-MM-yyyy", new Date())
  ...
  {selectedProof === "renewalnotice" && (
        <div>
          <label className="labelcls">Upload Proof Of Document</label>
          <input
            type="file"
            // {...register("proofOfdocumentfile")}
            className={inputClass}
          />
        </div>
      )}
