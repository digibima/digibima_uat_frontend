...
export default function StepperForm({ usersData, kycData }) {
  ...
  const updateUrlPremium = (total, base, coverage) => {
    ...
    params.set("totalPremium", total);
    params.set("basePremium", base);
  ...
  const handlePremiumUpdate = async (total, base, coverage) => {
    ...
    await Promise.all([
      ...
      deleteDBData(constant.DBSTORE.HEALTH.ULTIMATE.ULTIMATECHECKOUTTENUREDATA),

      deleteDBData(constant.DBSTORE.HEALTH.BAJAJ.BAJAJCHECKOUTDATA),
      deleteDBData(constant.DBSTORE.HEALTH.BAJAJ.BAJAJCHECKOUTTENUREDATA),
    ]);
  };

  useEffect(() => {
    const resume = sessionStorage.getItem("adityabirlamax_resume");

    if (!resume) return;

    const quote = sessionStorage.getItem("adityabirlamax_quote_summary");

    const final = sessionStorage.getItem("adityabirlamax_final_summary");
    ...
    sessionStorage.removeItem("adityabirlamax_resume");
  }, []);
...
const summaryData = useMemo(() => {
  ...
  const sessionDataStr = typeof window !== "undefined" ? sessionStorage.getItem("adityaBirlaMaxPlusSummaryCard") : null;
  const storedSummary = typeof window !== "undefined" ? sessionStorage.getItem("adityabirlamax_quote_summary") : null;
  
  if (sessionDataStr) {
    try {
      const parsedSession = JSON.parse(sessionDataStr);
      return {
        tenure: parsedSession.tenure || "",
        coverAmount: parsedSession.coverAmount || "",
        totalPremium: parsedSession.totalPremium || "",
        checkoutTotalPremium: parsedSession.checkoutTotalPremium || "",
        basepremium: parsedSession.basepremium || "",
        coverage: parsedSession.coverage || "",
        selectedAddons: parseIfString(parsedSession.selectedAddons),
        compulsoryAddons: parseIfString(parsedSession.compulsoryAddons),
        tenurePrices: parseIfString(parsedSession.tenurePrices),
        addons: parseIfString(parsedSession.addons),
        fullAddonsName: parseIfString(parsedSession.fullAddonsName),
      };
    } catch (e) {
      console.error("Error parsing sessionStorage journey data", e);
    }
  }
  ...
  const back = async () => {
    if (currentStep === 1) {
      router.push(constant.ROUTES.HEALTH.ADITYABIRLAMAX.CHECKOUT);
    } else {
      setLoading(true);
  ...
  const validateFormStepOne = async () => {
    ...
    try {
      ...
      const res = await CallApi(
        constant.API.HEALTH.ADITYABIRLAMAX.SAVESTEPONE,
        "POST",
        values,
      );
      if (res === 1 || res?.status) {
  ...
  const validateFormExtraStep = async () => {
    ...
    try {
      ...
      const res = await CallApi(
        constant.API.HEALTH.ADITYABIRLAMAX.SAVESTEPPORT,
        "POST",
        values,
      );
...
const validateFormStepThree = async (step3Form, steptwodata) => {
  try {
    ...
    // Helper function to check truthy values ("yes", "true", true, 1, "on")
    const isTruthy = (val) => {
      if (!val) return false;
    ...
    // =========================================================
    // 2. PED LIMIT CHECK (MANDATORY BLOCK)
    // =========================================================

    // A) Agar rule 3 MEMBERS se zyada ke paas PED na hone ka hai:
    const totalPedMembers = result.length;

    // B) Agar rule Total 3 PED DISEASES/QUESTIONS se zyada na hone ka hai:
    let totalPedQuestionsCount = 0;
    result.forEach((m) => {
      totalPedQuestionsCount += m.data ? m.data.length : 0;
    });

    // Agar 3 se zyada members ya 3 se zyada PED conditions hain:
    if (totalPedMembers > 3 || totalPedQuestionsCount > 3) {
      showError("Pre-Existing Diseases (PED) cannot be selected for more than 3 instances/members.");
      return false; // Yahin par stop ho jayega, aage nahi jayega
    }

    // =========================================================
    // 3. Field Invalid Error Check
    // =========================================================
    if (hasError) {
    ...
    // =========================================================
    const res = await CallApi(
      constant.API.HEALTH.ADITYABIRLAMAX.SAVESTEPTHREE, // Ya ADITYABIRLA
      "POST",
      result
    );
  ...
  const GoToPayment = async () => {
    ...
    try {
      const res = await CallApi(
        constant.API.HEALTH.ADITYABIRLAMAX.INITIATE,
        "POST",
      );
      ...
      if (status === "1" || status === 1 || status === true) {
        ...
        if (paymentUrl) {
          router.push(
            `/health/vendors/adityabirlamax/payment?paymentUrl=${encodeURIComponent(paymentUrl)}`,
          );
        } else {
...
// return false;
    const res = await CallApi(
      constant.API.HEALTH.ADITYABIRLAMAX.CREATEPOLICY,
      "POST",
      payload
    );
    ...
    // console.log("Create Policy Response =>", res);
     ...
     const status = res?.status;
       ...
       if (status === "1" || status === 1 || status === true) {
           router.push(
             `/health/vendors/adityabirlamax/payment/thankyou?policyNumber=${encodeURIComponent(policyNumber)}`,
           );
         } else {
           const fallbackMsg = "Something went wrong while creating policy.";
           const backendMsg =
  ...
  useEffect(() => {
    ...
    const fetchKycStatus = async () => {
      try {
        const res = await CallApi(
          constant.API.HEALTH.ADITYABIRLAMAX.KYCRESULT,
          "POST",
          { transactionId },
        );
  ...
  useEffect(() => {
    ...
    if (status === "CHARGED" && orderId) {
      ...
      const savedData = sessionStorage.getItem("adityaBirlaMaxStepFourData");
      ...
      CallApi(
        `${constant.API.HEALTH.ADITYABIRLAMAX.PAYMENTSTATUS}?order_id=${orderId}`,
        "GET",
      )
        .then((res) => {
  ...
  try {
    ...
    const cleanNumber = (val) => {
      ...
      if (typeof val === 'string') {
        return toNumber(val.replace(/,/g, '')); 
      }
      return toNumber(val);
    };
    
    setPrevPremiumBeforeMemberUpdate(
      cleanNumber(quoteData?.totalpremium || summaryData?.totalPremium || 0)
    );
    ...
    const res = await CallApi(
      constant.API.HEALTH.ADITYABIRLAMAX.UPDATEAGE,
      "POST",
      payload,
    );
    ...
    if (res === 1 || res?.status) {
      ...
      if (typeof clearDBData === "function") {
        await clearDBData();
      } else {
        console.warn("clearDBData function is not defined");
      }
