...
const handleOcrData = (data) => {
            ...
            if (!fetchedPincode) {
              ...
              try {
                ...
                if (quoteResponse?.status) {
                  ...
                  //   deleteDBData(
                  //     constant.DBSTORE.HEALTH.PLANS.HEALTHPLANVENDOR,
                  //   ),
                  //   deleteDBData(constant.DBSTORE.HEALTH.CARE.CARECHECKOUTDATA),
                  //   deleteDBData(
                  //     constant.DBSTORE.HEALTH.CARE.CARECHECKOUTTENUREDATA,
                  //   ),
                  ...
                  //   ),
                  //   deleteDBData(
                  //     constant.DBSTORE.HEALTH.ULTIMATE
                  //       .ULTIMATECHECKOUTTENUREDATA,
                  //   ),

                  //   deleteDBData(
                  //     constant.DBSTORE.HEALTH.BAJAJ.BAJAJCHECKOUTDATA,
                  //   ),
                  //   deleteDBData(
                  //     constant.DBSTORE.HEALTH.BAJAJ.BAJAJCHECKOUTTENUREDATA,
                  //   ),
                  // ]);
                   await clearDBData();
                }
  ...
  useEffect(() => {
    ...
    fetchDataONE();
  }, [step1Form,setInsureMemberData]);

  useEffect(() => {
const isKycApplied = sessionStorage.getItem("kyc_applied");

if (!usersData || isUserPrefilled || isKycApplied) return;
    ...
    // sessionStorage.setItem("planType",usersData.plantype,
    // );

    setPortTenure(usersData.tenure);
   const insure = usersData.insuredata;

if (Array.isArray(insure)) {
  setSelfData([
    {
      age: Number(insure[0]?.age || insure[0]?.Age || ""),
      gender: (insure[0]?.gender || insure[0]?.Gender || "").toLowerCase(),
    },
  ]);
} else if (insure && typeof insure === "object") {
  setSelfData([
    {
      age: Number(insure.age || insure.Age || ""),
      gender: (insure.gender || insure.Gender || "").toLowerCase(),
    },
  ]);
}
   ...
   if (user.dob && !sessionStorage.getItem("kyc_applied")) {
    ...
    if (
      ...
      if (gender) {
        ...
        step1Form.setValue("mr_ms_gender", mappedGender, {
          shouldValidate: true,
        });
      }
  ...
  useEffect(() => {
    ...
    Object.entries(map).forEach(([apiKey, formKey]) => {
      ...
      if (verifiedData[apiKey] && !isDirty) {
        if (apiKey === "dob") {
          ...
          set(formKey, verifiedData.gender);
        } else {
    ...
    // Lock only if PAN verified
    if (verifiedData?.kyctype === "p") {
      setIsKycLocked(true);
    }
  }, [
    verifiedData,
  ...
  ${type === "Aadhar ( Last 4 Digits )" || type === "Others"
      ...
      : "border-gray-400 bg-white"
          ...
          >
            ...
            <input
              ...
              // disabled={type === "Aadhar ( Last 4 Digits )" || type === "Others" }
