...
export default function StepTwoForm({
  ...
  const members = steponedata?.members || [];

  const orderedMembers = useMemo(() => {
    const spouseMembers = members.filter((m) =>
      ["wife", "husband"].includes(m.name?.toLowerCase())
    );

    const childrenMembers = members.filter((m) =>
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await CallApi(
          constant.API.HEALTH.ADITYABIRLAMAX.SAVESTEPTWO,
          "GET"
        );
        ...
        const savedOcc = propDetailsObj?.occupation || selfData?.occupation || "";
        if (savedOcc && occupationlist.length > 0) {
          ...
          step2Form.setValue("proposeroccupation", occVal, { shouldValidate: true });
        }

        const savedDuty = propDetailsObj?.nature_of_duty || selfData?.nature_of_duty || "";
        if (savedDuty && natureDuty.length > 0) {
          ...
          step2Form.setValue("natureofduty", dutyVal, { shouldValidate: true });
        }
        ...
        if (nomineeData) {
          ...
          if (nomineeData.appointee_relation) {
            step2Form.setValue("appointeerelation", nomineeData.appointee_relation);
          }
  ...
  useEffect(() => {
    ...
    orderedMembers.forEach((member) => {
      ...
      const apiMember = apiMembers.find((api) => {
        ...
        if (
          apiRel === relation ||
          (isChild && (apiRel.includes("child") || apiRel === "son" || apiRel === "daughter"))
        ) {
          matchIndex += 1;
      ...
      // Member Occupation Fix
      if (!isChild && occupationdata.length > 0) {
        const mOcc =
      ...
      // Member Nature of Duty Fix
      if (!isChild && natureofdutydata.length > 0) {
        const mDuty =
  ...
  }, [apiMembers, JSON.stringify(steponedata?.members), planType, step2Form, occupationdata, natureofdutydata]);
    ...
    <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            {...step2Form.register("proposerdob2", { required: "Proposer DOB required" })}
            readOnly={true}
        ...
        {planType == 2 && (
          <div className="flex flex-col gap-1">
            ...
            <Controller
              ...
              rules={{ required: planType == 2 ? "Proposer Attachment Date required" : false }}
        ...
        <div className="flex flex-col gap-1">
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="annualincome">
            Annual Income
          </label>
          <input
            id="annualincome"
            {...step2Form.register("annualincome", { required: "Annual Income is required" })}
            onChange={(e) => isNumber(e, step2Form.setValue, "annualincome")}
            maxLength={8}
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex gap-2">
            <input
              {...step2Form.register("proposerheight", { required: "Height (Feet) is required" })}
            ...
            />
            <input
              {...step2Form.register("proposerinches", { required: "Height (Inches) is required" })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            {...step2Form.register("proposarbankaccount", { required: "Bank Account is required" })}
        ...
        <div className="flex flex-col gap-1">
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="proposaraccounttype">
            Account Type
          </label>
          <select
            id="proposaraccounttype"
            {...step2Form.register("proposaraccounttype", { required: "Account Type is required" })}
            className={inputClass}
          >
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            id="proposarbankifsc"
            {...step2Form.register("proposarbankifsc", { required: "Bank IFSC is required" })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1">
                ...
                <input
                  {...step2Form.register(
                    ...
                    { required: isSpouse ? `${spouseKeyLabel}name is required` : "Name is required" }
                  )}
                  placeholder={isSpouse ? `Enter ${spouseLabel}'s Full Name` : `Enter ${member.name}'s Full Name`}
                  className={inputClass}
                />
              </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  {...step2Form.register(dobFieldName, {
                    required: isSpouse ? `${spouseKeyLabel}dob is required` : "Please select a valid date",
                  })}
                  readOnly={true}
              ...
              {planType == 2 && (
                <div className="flex flex-col gap-1">
                  ...
                  <Controller
                    control={step2Form.control}
                    name={isChild ? `${prefix}attachmentdate${suffix}` : `${prefix}attachmentdate`}
                    rules={{ required: planType === 2 ? "Attachment Date is required" : false }}
                    render={({ field, fieldState }) => (
                      <UniversalDatePicker
                        id={isChild ? `${prefix}attachmentdate${suffix}` : `${prefix}attachmentdate`}
                        name={isChild ? `${prefix}attachmentdate${suffix}` : `${prefix}attachmentdate`}
                        value={field.value ? parse(field.value, "dd-MM-yyyy", new Date()) : null}
                        onChange={(date) => {
                          if (date instanceof Date && !isNaN(date)) {
                            ...
                            handleDateChange(
                              `${prefix}${suffix}`,
                              isChild ? `${prefix}attachmentdate${suffix}` : `${prefix}attachmentdate`
                            )(date);
                          }
                        }}
                        placeholder="Select Date"
                ...
                <div className="flex gap-2">
                  <input
                    {...step2Form.register(
                      ...
                      { required: isSpouse ? `${spouseKeyLabel}height is required` : "Height (Feet) is required" }
                    )}
                    onChange={(e) => isNumber(e, step2Form.setValue, isChild ? `${prefix}height${suffix}` : `${prefix}height`)}
                    maxLength={1}
                    placeholder="Feet"
                    className={`${inputClass} w-1/2`}
                  />
                  <input
                    {...step2Form.register(
                      ...
                      { required: isSpouse ? `${spouseKeyLabel}inches is required` : "Height (Inches) is required" }
                    )}
                    onChange={(e) => isNumber(e, step2Form.setValue, isChild ? `${prefix}inches${suffix}` : `${prefix}inches`)}
                    maxLength={2}
                    placeholder="Inches"
                    className={`${inputClass} w-1/2`}
                  />
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  {...step2Form.register(
                    ...
                    { required: isSpouse ? `${spouseKeyLabel}weight is required` : "Weight is required" }
                  )}
                  onChange={(e) => isNumber(e, step2Form.setValue, isChild ? `${prefix}weight${suffix}` : `${prefix}weight`)}
                  maxLength={3}
                  placeholder="Enter Weight"
                  className={inputClass}
                />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="flex flex-col gap-1">
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="nomineegender">
            Gender
          </label>
          <select
            id="nomineegender"
            {...step2Form.register("nomineegender", { required: "Nominee Gender is required" })}
            className={inputClass}
          >
        ...
        <div className="flex flex-col gap-1">
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="nomineename">
            Nominee Full Name
          </label>
          <input
            id="nomineename"
        ...
        <div className="flex flex-col gap-1">
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="nomineerelation">
      ...
      {isNomineeMinor && (
        <>
          <h2 className="text-xl font-bold text-gray-800 mt-8">Appointee Details:</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="appointeename">
                Appointee Full Name
              </label>
              <input
                id="appointeename"
                {...step2Form.register("appointeename", {
                  required: isNomineeMinor ? "Appointee Name is required" : false,
                })}
            ...
            <div className="flex flex-col gap-1">
              ...
              <Controller
                control={step2Form.control}
                name="appointeedob"
                rules={{ required: isNomineeMinor ? "Appointee DOB is required" : false }}
                render={({ field, fieldState }) => (
                  <UniversalDatePicker
                    id="appointeedob"
                    name="appointeedob"
            ...
            <div className="flex flex-col gap-1">
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="appointeerelation">
                Relationship with Nominee
              </label>
              <select
                id="appointeerelation"
                {...step2Form.register("appointeerelation", {
                  required: isNomineeMinor ? "Please select Appointee relation" : false,
                })}
                className={inputClass}
              >
      ...
      <button type="button" onClick={onSubmitStep} className="mt-6 px-6 py-2 thmbtn">
        Continue
      </button>
    </form>
  );
