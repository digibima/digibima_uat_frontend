...
export default function PolicyPeriodOptions({
  ...
  // Discount calculate / return karne ke liye helper function
  const getDiscountBadge = (year) => {
    const y = Number(year);
    if (y === 2) return "7.5% OFF";
    if (y === 3) return "10% OFF";
    return null;
  };
  ...
  if (isSkeletonLoading || tenureOptions.length === 0) {
    return (
  ...
  return (
    <div className="bg-white rounded-xl p-4 px-5 mb-6">
      ...
      <div className="flex flex-wrap gap-4 pt-2 mb-2">
        {tenureOptions.map((year) => {
          const discountText = getDiscountBadge(year);
          ...
          return (
            ...
            >
              {/* Ping Animation on Selected Tenure */}
              {tenure == year && (
              ...
              {/* Discount Badge */}
              {discountText && (
                <span className="absolute -top-3 right-3 bg-emerald-600 text-white text-[11px] font-bold px-2 py-0.5 rounded-full shadow-sm">
                  {discountText}
                </span>
              )}
