...
export default function InsureSidebarComponent({ onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    const formData = [...childList, ...memberList];
     illnessMutation.mutate(formData);
  };
  const queryClient = useQueryClient();

const illnessMutation = useMutation({
  mutationFn: (payload) =>
    CallApi(constant.API.HEALTH.ILLNESS, "POST", payload),
  onSuccess: () => {
  ...
  onError: () => showError("Failed to submit data. Please try again."),
});
