...
export default function SummaryCard({
  ...
  // console.log("right section data", selectedAddons);
  // console.log("right fullAddonsName", fullAddonsName);
  // console.log("right addons", addons);
  ...
  const isCheckoutPage = pathname.includes(
    "/health/vendors/adityabirla/checkout",
  );
  const isJourneyPage = pathname.includes(
    "/health/vendors/adityabirla/journey",
  );
  const [isResume, setIsResume] = useState(false);

  useEffect(() => {
    const resume = sessionStorage.getItem("adityabirla_resume") === "1";
    setIsResume(resume);
  }, []);
 ...
 useEffect(() => {
    // Agar checkout ya total premium dono hi nahi hain toh message khali rakhein
    if (!checkoutTotalPremium && !totalPremium) {
      setPriceChangeMsg("");
      return;
    }
    ...
    if (isMemberUpdated && prevPremiumBeforeMemberUpdate) {
      const oldPrice = formatPrice(normalize(prevPremiumBeforeMemberUpdate));
 ...
 const summaryData = {
  ...
  router.push("/health/vendors/adityabirla/journey");
  };
