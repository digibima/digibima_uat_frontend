      ...
      <div className="flex flex-wrap gap-4 mb-2">
        {tenureOptions.map((year) => {
          ...
          return (
            <label
              key={year}
              className={`relative flex items-center justify-between gap-2 px-6 py-3 border rounded-xl cursor-pointer transition-all duration-200 flex-1 min-w-[160px] max-w-full ${
                tenure == year ? "border-pink-500 bg-pink-50/10" : "border-gray-400"
              }`}
            >
              {/* Ping Animation on Active Option */}
              {tenure == year && (
                <span className="absolute -top-1 -right-1 w-3 h-3 bg-pink-400 rounded-full animate-ping opacity-70" />
              )}

              {/* Discount Tag Top-Right Badge */}
              {numericYear === 2 && (
                <span className="absolute -top-2.5 right-3 bg-emerald-600 text-white text-[11px] font-bold px-2 py-0.5 rounded-full shadow-sm">
                  Discount 7.5%
                </span>
              )}
