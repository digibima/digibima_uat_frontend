...
export default function ProposalUI() {
  ...
  useEffect(() => {
    const fetchCheckoutData = async () => {
      ...
      try {
        const cached = await getDBData(constant.DBSTORE.HEALTH.ADITYABIRLA.ABIRLACHECKOUTDATA);
        ...
        const res = await CallApi(constant.API.HEALTH.ADITYABIRLA.CHECKOUT);
        if (res) {
          populateData(res);
          setOriginalData(res);
          await storeDBData(constant.DBSTORE.HEALTH.ADITYABIRLA.ABIRLACHECKOUTDATA, res);
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchPrices = async () => {
      const cacheKey = constant.DBSTORE.HEALTH.ADITYABIRLA.ABIRLACHECKOUTTENUREDATA;
        ...
        for (const t of tenureOptions) {
          try {
            ...
            const res = await CallApi(
              constant.API.HEALTH.ADITYABIRLA.PlANCHECKOUT,
              "POST",
