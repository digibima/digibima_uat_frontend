...
const formatAmount = (amt) => {
  ...
  if (String(amt).toLowerCase() === "unlimited") {
    return "Unlimited";
  }
...
router.push("/health/vendors/ultimatecare/journey");
  };

  const handleBrowse = () => {
    window.open(
      "https://stage.digibima.com/broucher/ultimatecare.pdf",
      "_blank"
    );
  };
    ...
    <div className="w-full lg:w-[415px] bg-white rounded-[32px] shadow-sm p-6 text-sm self-start">
      ...
      <div className="space-y-1 text-gray-700">
        {compulsoryAddons.length > 0 ? (
          compulsoryAddons
  .filter((key) => {
    if (String(coverage || coverAmount).toLowerCase() === "unlimited") {
      return !["tm", "uc", "ib"].includes(key);
    }
    return true;
  })
  .map((key) => (
  ...
  .map(([k, val]) => {
             ...
             if (valueStr === "opdplus") {
                displayKey = "opdp"; // full form key
                rightSide = formatPrice(addons.opd || 0);
              } 
              else if (isOpdVariant(valueStr)) {
                displayKey = "opd";
                rightSide = formatPrice(addons.opd || 0);
              }
      ...
      <div>
       ...
       {isJourneyPage && (
  (isPaymentFailed ||(planType == 2 && currentStep === 5) || (planType != 2 && isStepFour)) && (
    <button
