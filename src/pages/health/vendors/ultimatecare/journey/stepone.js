...
const handleOcrData = (data) => {
  ...
  const handlePincodeInput = useCallback(
    ...
    [fetchedPincode, step1Form, setOldPincode, setNewPincode, setQuoteData,updateUrlPremium],
  );
...
if (Array.isArray(members)) {
  ...
  if (selfMember) {
    ...
    const liveDob = targetSelf?.dob || user.dob; 
...
useEffect(() => {
  ...
  if (!currentVerifiedData || Object.keys(currentVerifiedData).length === 0) {
    const savedKyc = sessionStorage.getItem("ultimate_verified_kyc_data");
    if (savedKyc) {
      try {
