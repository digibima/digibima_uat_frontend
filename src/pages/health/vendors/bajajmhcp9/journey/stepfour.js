...
export default function StepFourForm({
  ...
  const handleEditStep = (stepNo) => {
    ...
    router.push(`/health/vendors/bajajmhcp9/journey?${currentParams.toString()}`);
  };
      ...
      <div className="overflow-hidden border border-gray-200 rounded-xl">
        <table className="min-w-full text-sm">
          <thead className="bg-gradient-to-r from-red-50 to-white">
            <tr>
              <th className="px-4 py-3 text-left font-semibold text-gray-700 w-[70px]">
                #
              </th>
            ...
            {medicalHistory.map((item, i) => (
              ...
              >
                <td className="px-4 py-4">
                  <div className="w-7 h-7 rounded-full bg-red-100 text-red-600 text-xs font-bold flex items-center justify-center">
                    {i + 1}
                  </div>
                </td>

               <td className="px-4 py-4 font-medium text-gray-800 max-w-[300px]">
  <ReadMoreText text={item.questionLabel} lines={2} />
</td>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {lifestyleHistory.map((item, i) => (
              <tr
                key={i}
                className="border-t hover:bg-yellow-50/40 transition"
              >
                <td className="px-4 py-4">
                  <div className="w-7 h-7 rounded-full bg-yellow-100 text-yellow-700 text-xs font-bold flex items-center justify-center">
                    {i + 1}
                  </div>
                </td>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {items.map(([label, value], i) => (
        <div
          key={i}
          className="bg-gradient-to-br from-gray-50 to-white border border-gray-200 rounded-xl p-4 hover:shadow-sm transition"
        >
