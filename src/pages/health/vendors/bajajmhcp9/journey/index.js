...
useEffect(() => {
  ...
  if (storedData) {
    ...
    const back = async () => {
      if (currentStep === 1) {
        router.push(constant.ROUTES.HEALTH.BAJAJMHCP9.CHECKOUT);
      } else {
        setLoading(true);
...
//  return;
    ...
    const validateFormStepTwo = async () => {
      ...
      try {
        setLoading(true);
        const res = await CallApi(
          constant.API.HEALTH.BAJAJMHCP9.SAVESTEPTWO,
          "POST",
          values
        );
        if (res === 1 || res?.status) {
    ...
    try {
      ...
      const res = await CallApi(
        constant.API.HEALTH.BAJAJMHCP9.SAVESTEPPORT,
        "POST",
        values
      );
...
const validateFormStepThree = async (step3Form, steptwodata) => {
  ...
  try {
      ...
      const processQuestions = (questions) => {
        Object.values(questions).forEach((q) => {
          ...
          if (toggle && selectedMembers.includes(m.id)) {
            ...
            showError(
              `Please enter remark for ${q.label}`
            );
    ...
    const res = await CallApi(
      constant.API.HEALTH.BAJAJMHCP9.SAVESTEPTHREE,
      "POST",
      transformed
    );
    ...
    const GoToPayment = async () => {
      setLoading(true);
      try {
        const res = await CallApi(constant.API.HEALTH.BAJAJMHCP9.CREATEPOLICY, "POST");
        ...
        if (status === "1" || status === 1 || status === true) {
          ...
          if (paymentLink) {
            router.push(
              `/health/vendors/bajajmhcp9/payment?paymentLink=${paymentLink}`
            );
          } else {
            const backendMsg =
  ...
  const onSubmitStep = async () => {
    ...
    useEffect(() => {
      const fetchFromDB = async () => {
        try {
          const data = await getDBData(constant.DBSTORE.HEALTH.BAJAJMHCP9.TENURETXN);
          if (data) {
