...
export default function StepperForm({ usersData, kycData }) {
  ...
  const updateUrlPremium = (total, base, coverage) => {
    const existing = JSON.parse(
      sessionStorage.getItem("careSuperemeSummaryCard") || "{}",
    );

    sessionStorage.setItem(
      "careSuperemeSummaryCard",
      JSON.stringify({
        ...existing,
        totalPremium: total,
        basepremium: base,
  ...
  useEffect(() => {
    const resume = sessionStorage.getItem("care_resume");

    if (!resume) return;

    const quote = sessionStorage.getItem("care_quote_summary");

    const final = sessionStorage.getItem("care_final_summary");
    ...
    sessionStorage.removeItem("care_resume");
  }, []);
  ...
  const summaryData = useMemo(() => {
    ...
    try {
      const storedData = sessionStorage.getItem("careSuperemeSummaryCard");
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem("care_quote_summary", JSON.stringify(quoteSummary));
  ...
  const validateFormStepOne = async () => {
    ...
    if (normalizedForm !== normalizedApi) {
      ...
      return false; // Yeh user ko next step par jaane se rok dega
    }

    // =============================================================================
    // --- MISMATCH VALIDATION END ---

    //  return false;   // --- GENDER MISMATCH VALIDATION END ---
  ...
  const validateFormExtraStep = async () => {
    ...
    try {
      ...
      const res = await CallApi(
        constant.API.HEALTH.CARESUPEREME.SAVESTEPPORT,
        "POST",
        values,
      );
    ...
    try {
      ...
      if (status === "1" || status === 1 || status === true) {
        ...
        if (response?.proposalNumber) {
          router.push(
            `/health/vendors/caresupereme/payment?proposalNumber=${proposalNumber}&proposalEncode=${proposalEncode}&guid=${guid}&returnURL=${returnURL}&gatewayURL=${gatewayURL}`,
          );
        }
      } else {
    ...
    try {
      ...
      if (currentStep === 1) {
        ...
        if (planType == "1") {
          ...
          isValid = await portquestionvalidation(
            step3Form,
            steptwodata,
            setStepThreeData,
          );
      ...
      <Modal
       ...
       onConfirm={async () => {
        try {
          ...
          console.log(payload);
          const res = await CallApi(
            constant.API.HEALTH.CARESUPEREME.UPDATEAGE,
            "POST",
            payload,
          );
          ...
          if (res === 1 || res?.status) {
            ...
            sessionStorage.setItem("careSuperemeSummaryCard", JSON.stringify(updatedSummary));
            sessionStorage.setItem("care_quote_summary", JSON.stringify(updatedSummary));
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <Image
                          src={
                            g === "male"
                              ? "/images/health/insure/father.jpg"
                              : "/images/health/insure/wife.jpg"
                          }
                          alt={g}
                          fill
                          sizes="40px"
