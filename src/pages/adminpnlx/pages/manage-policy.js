...
export default function Policy() {
  ...
  accessorKey: "action",
  cell: ({ row }) => {
    const hasPolicyNumber = row.original?.policyNumber;
    const hasPdf = row.original?.policy_pdf;

    return hasPolicyNumber && hasPdf ? (
      <Link
        href={hasPdf}
        target="_blank"
      ...
      <FaDownload
        onClick={() =>
          showError("Policy PDF link not available")
        }
  ...
  const handleDownload = async (policy) => {
    try {
      ...
      const res = await CallApi(
        constant.API.USER.DOWNLOADPOLICY,
        "POST",
